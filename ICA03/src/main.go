...
func main() {
	...
	for scanner.Scan() {
		...
		case "3c":
			...
			fmt.Println("Her er pakken golang.org/x/text/encoding/charmap importert.")
			oppgave3c();
			ferdig();
			break
		case "4a":
...
func oppgave3c() {
	...
	// Use charmap library to convert
	iso, err := charmap.ISO8859_1.NewDecoder().Bytes(byteslice)
	if err != nil {
		fmt.Println("ERROR")
	}
	...
	fmt.Printf("%s", iso)
	fmt.Println("")
	fmt.Println("Endelig resultat;")
	fmt.Println(treasure.PrintTreasureUTF8(treasureString))
...
func oppgave4a() {
	...
	fmt.Println(unicode.Translate(tekst, "Norwegian"))
	fmt.Println(unicode.Translate(tekst, "Icelandic"))
	fmt.Println(unicode.Translate(tekst, "Japanese"))
